...
function customer_settings($selected_id) {
	...
	//Added check if Walk-In Customer is selected then display not editable fields.
	if($selected_id == 1){
		text_row(_('Customer Name:'), 'CustName', $_POST['CustName'], 40, 80, '', '','','readonly');
		text_row(_('Customer Short Name:'), 'cust_ref', $_POST['cust_ref'], 30, 30, '', '','','readonly');
	}
	else{
	...
	if (!$selected_id)
		...
		// Start - MUZZAMMIL - 09-Mar-2023
		// Added a check Only display "Delete Customer" button when customer ID is not 3 i.e Walk-In Customer
		if ($selected_id != 1) {
			submit_center_last('delete', _('Delete Customer'), _('Delete customer data if have been never used'), true);
		}
		// End - MUZZAMMIL - 09-Mar-2023
