		...
		if (count($_SESSION['PO']->line_items) > 0) {
			foreach ($_SESSION['PO']->line_items as $order_item) {
				...
				if (($order_item->stock_id == $_POST['stock_id'])){
					//Start - MUZZAMMIL - 03-Mar-2023
					//adding logic of Item is IMEI based then return false do not add again
					if(strlen($_POST['stock_id']) == 15){
						display_warning(_('IMEI:').$_POST['stock_id'].' '._('is already added.'));
						return false;
					}
					//End - MUZZAMMIL - 03-Mar-2023
...
function can_commit() {
	...
	if (!get_post('supplier_name'))	{
		display_error(_('Supplier name is required.'));
		set_focus('supplier_name');
		return false;
	}
