...
function supplier_settings(&$supplier_id) {
	...
	//Added check if Walk-In Supplier is selected then display not editable fields.
	if($supplier_id == 1){
		text_row(_('Supplier Name:'), 'supp_name', $_POST['supp_name'], 42, 60, '', '','','readonly');
		text_row(_('Supplier Short Name:'), 'supp_ref', $_POST['supp_ref'], 30, 30, '', '','','readonly');
	}
	else{
	...
	if ($supplier_id) {
		...
		// Added a check Only display "Delete Supplier" button when supplier ID is not 3 i.e Walk-In Supplier
		if ($supplier_id != 1) {
